...
(function () {
  ...
  function buildNavbar() {
    ...
    <button class="btn btn--ghost btn--sm" onclick="logout()" style="margin-left:10px;color:var(--red-accent);">
      🚪 Salir
    </button>
  `;
    }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      !e.target.closest('button[onclick="logout()"]')
    ) {
  ...
  async function loadNavBalance() {
    try {
      ...
      const bal =
        parseFloat(data.wallet.balance) -
        parseFloat(data.wallet.lockedBalance || 0);
